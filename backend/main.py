...
async def run_bounded_image_job(job_name: str, func, *args, **kwargs):
    ...
    try:
        await asyncio.wait_for(
            IMAGE_JOB_SEMAPHORE.acquire(), timeout=IMAGE_JOB_ACQUIRE_TIMEOUT_SECONDS
        )
...
def send_verification_code_email(recipient_email: str, verification_code: str) -> None:
    ...
    message.set_content("""Your confirmation code is below.
...
async def register(
    ...
    if existing_user:
        audit_auth_event(
            "register", request, "failure", email=user_data.email, detail="email_exists"
        )
    ...
    if not EMAIL_VERIFICATION_REQUIRED:
        ...
        audit_auth_event(
            "register", request, "success", user_id=user.id, email=user.email
        )
        return {
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail="Unable to send confirmation email right now."
        )
...
async def login(
    ...
    if not email or "@" not in email:
        audit_auth_event(
            "login", request, "failure", email=email, detail="invalid_email_format"
        )
        raise HTTPException(status_code=400, detail="Invalid email format")
    if not password:
        audit_auth_event(
            "login", request, "failure", email=email, detail="missing_password"
        )
    ...
    if not user or not user.password_hash:
        audit_auth_event(
            "login", request, "failure", email=email, detail="invalid_credentials"
        )
...
async def refresh_token(
    ...
    if token_record.revoked_at is not None:
        audit_auth_event(
            "refresh", request, "failure", user_id=int(user_id), detail="token_replay"
        )
    ...
    if not user:
        audit_auth_event(
            "refresh", request, "failure", user_id=int(user_id), detail="user_not_found"
        )
    ...
    return build_auth_response(
        db, user.id, request=request, previous_refresh_jti=str(jti)
    )
...
async def logout(
    ...
    try:
        ...
        audit_auth_event(
            "logout", request, "success", user_id=int(user_id) if user_id else None
        )
        ...
        audit_auth_event(
            "logout", request, "failure", detail="invalid_or_expired_refresh_token"
        )
...
async def revoke_auth_session(
    ...
    revoked = crud.revoke_user_refresh_token(
        db, current_user_id, jti, reason="manual_revoke"
    )
    if not revoked:
        audit_auth_event(
            "session_revoke",
...
async def confirm_password_reset(
    ...
    try:
        ...
        if not user_id:
            audit_auth_event(
                "password_reset_confirm", request, "failure", detail="missing_user_id"
            )
        ...
        audit_auth_event(
            "password_reset_confirm", request, "failure", detail="invalid_token"
        )
        raise HTTPException(status_code=401, detail="Invalid reset token")
...
async def retrieve_image(
    ...
    try:
        ...
        logger.info(
            f"Image retrieval: user {current_user_id} accessed image {image_id}"
        )
        ...
        logger.warning(
            f"Unauthorized image access: user {current_user_id} tried to access {image_id}"
        )
        raise HTTPException(status_code=403, detail="Access denied")
    ...
    try:
        ...
        if INFERENCE_QUEUE_ENABLED:
            try:
                ...
                raise HTTPException(
                    status_code=503, detail="Inference queue unavailable"
                )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Image storage failed: {str(e)}"
            )
...
async def delete_wardrobe_item_endpoint(
    ...
    item = (
        ...
        .filter(
            models.WardrobeItem.id == item_id, models.WardrobeItem.user_id == user_id
        )
        .first()
    )
    if not item:
...
async def get_user_feedback_summary(
    ...
    helpful_feedback_count = sum(
        1 for feedback in recommendation_feedback if bool(feedback.helpful)
    )
    average_outfit_rating = (
        round(sum(rating.rating for rating in outfit_ratings) / len(outfit_ratings), 2)
        if outfit_ratings
        else None
    )
    helpful_rate = (
        round(helpful_feedback_count / len(recommendation_feedback), 2)

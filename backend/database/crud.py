        ...
        for idx, item in enumerate(data["items"]):
            ...
            if "id" not in item or not isinstance(item["id"], int) or item["id"] <= 0:
                raise ValidationError(
                    f"Item {idx} must have valid 'id' (positive integer)"
                )
            ...
            if (
                ...
                raise ValidationError(
                    f"Item {idx} must have valid 'quantity' (positive integer)"
                )
...
def _check_user_authorization(
    db: Session, target_user_id: int, requester_id: int
) -> None:
    """
    ...
    logger.warning(
        f"Unauthorized access attempt: requester {requester_id} (role: {requester_role}) accessing user {target_user_id}"
    )
...
def revoke_user_refresh_token(
    db: Session, user_id: int, jti: str, reason: str = "manual_revoke"
) -> bool:
...
def create_wardrobe_item(
    ...
    try:
        ...
        raise DatabaseError(
            f"Failed to create wardrobe item: Database constraint violation"
        )
...
def delete_wardrobe_item(
    db: Session, item_id: int, requester_id: Optional[int] = None
) -> bool:
    """
...
def get_user_outfits(
    ...
    try:
        ...
        # Get total count
        total = (
            db.query(func.count(Outfit.id)).filter(Outfit.user_id == user_id).scalar()
        )
...
def create_outfit_rating(
    ...
    try:
        ...
        outfit = (
            ...
            .filter(models.Outfit.id == outfit_id, models.Outfit.user_id == user_id)
            .first()
        )
        ...
        logger.info(
            f"Created outfit rating: user={user_id}, outfit={outfit_id}, rating={rating}"
        )
...
def create_recommendation_feedback(
    ...
    try:
        ...
        logger.info(
            f"Created recommendation feedback: user={user_id}, type={recommendation_type}, helpful={helpful}"
        )
        return feedback
...
def create_item_usage(
    ...
    try:
        ...
        if not item:
            raise ValidationError(
                f"Wardrobe item {item_id} not found for user {user_id}"
            )
        ...
        logger.info(
            f"Created item usage: user={user_id}, item={item_id}, action={action}"
        )
        return usage
...
def create_model_metric(
    ...
    try:
        ...
        metric = models.ModelMetrics(
            model_name=model_name, metric_type=metric_type, value=value, version=version
        )
        ...
        logger.info(
            f"Created model metric: model={model_name}, type={metric_type}, value={value}"
        )
        return metric
...
def get_feedback_for_period(
    db: Session, days: int = 30, limit: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        ratings_q = db.query(models.OutfitRating).filter(
        ...
        rec_feedback_q = db.query(models.RecommendationFeedback).filter(
        ...
        usage_q = db.query(models.ItemUsage).filter(
...
def get_model_metrics(
    db: Session, model_name: str, limit: int = 10

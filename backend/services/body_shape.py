...
def _assess_pose_quality(
    keypoints_data: np.ndarray, img_width: int, img_height: int
) -> Dict:
    ...
    visible_count = sum(
        1 for i in VISIBILITY_KEYPOINTS if float(keypoints_data[i][2]) > 0.3
    )
    visible_ratio = visible_count / len(VISIBILITY_KEYPOINTS)

    # Check if person bounding box is reasonable (not too close to edges)
    xs = [
        float(keypoints_data[i][0])
        for i in VISIBILITY_KEYPOINTS
        if float(keypoints_data[i][2]) > 0.3
    ]
    ys = [
...
def calculate_body_measurements(keypoints):
    ...
    is_anatomically_valid = (
        0.3 < shoulder_hip_ratio < 3.5  # Reasonable ratio
        and shoulder_width > 20
        and hip_width > 20  # Minimum size
        and waist_width > 0
    )

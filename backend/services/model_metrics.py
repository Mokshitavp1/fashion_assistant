...
def evaluate_outfit_accuracy(
    db: Session, lookback_days: int = 30, min_ratings: int = 10
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if len(ratings) < min_ratings:
            logger.info(
                f"Insufficient outfit ratings ({len(ratings)} < {min_ratings}) for evaluation"
            )
            return {
                "status": "skipped",
        ...
        for rating in ratings:
            try:
                ...
                outfit = (
                    ...
                    .filter(models.Outfit.id == rating.outfit_id)
                    .first()
                )
                ...
                logger.debug(
                    f"Error comparing prediction for outfit {rating.outfit_id}: {e}"
                )
                continue
        ...
        # Compute metrics
        mae = (
            total_absolute_error / valid_comparisons
        )  # Mean absolute error (lower is better)
        accuracy = (
            rating_agreement_sum / valid_comparisons
...
def evaluate_recommendation_helpful_rate(
    db: Session, lookback_days: int = 30, rec_type: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not rec_feedback:
            ...
            return {"status": "skipped", "reason": "no_data", "feedback_count": 0}

        # Filter by recommendation type if specified
        if rec_type:
            rec_feedback = [
                f for f in rec_feedback if f.recommendation_type == rec_type
            ]
            if not rec_feedback:
                return {
                    "status": "skipped",
        ...
        try:
            model_name = (
                f"recommendations_{rec_type}" if rec_type else "recommendations"
            )
            crud.create_model_metric(
                db=db,
            ...
            logger.info(
                f"Stored helpful_rate metric for {model_name}: {helpful_rate:.3f}"
            )
        except Exception as e:
...
def evaluate_model_drift(
    db: Session, model_name: str, lookback_days: int = 30, drift_threshold: float = 0.1
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if len(accuracy_metrics) < 10:
            logger.info(
                f"Insufficient accuracy metrics for {model_name} drift detection"
            )
            return {
                "status": "skipped",
        ...
        if is_drifting:
            try:
                ...
                logger.warning(
                    f"Model drift detected for {model_name}: {drift_score:.3f}"
                )
            except Exception as e:

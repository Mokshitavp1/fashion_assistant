...
def find_matching_wardrobe_items(
    new_item_color: str, new_item_category: str, wardrobe_items: List[WardrobeItem]
) -> List[Dict]:
    """
    ...
    new_item_category = _require_non_empty_string(
    ...
    if new_item_category == "top":
        ...
        potential_matches = categorized_items.get("top", []) + categorized_items.get(
            "bottom", []
        )
...
def calculate_wardrobe_compatibility_score(
    matching_items: List[Dict], total_relevant_items: int
) -> Dict:
    """
    ...
    if match_count > 0:
        ...
        avg_harmony = (
            sum(item["harmony_score"] for item in matching_items) / match_count
        )
...
def check_duplicate_in_wardrobe(
    new_item_color: str, new_item_type: str, wardrobe_items: List[WardrobeItem]
) -> Dict:
    """
    ...
    for item in wardrobe_items:
        ...
        if (
            item.color_primary.lower() == new_item_color.lower()
            and item.clothing_type.lower() == new_item_type.lower()
        ):
    ...
    return {"is_duplicate": len(similar_items) > 0, "similar_items": similar_items}
...
def generate_purchase_recommendation(
    ...
    if compatibility_score >= 0.6:
        reasons.append(
            f"✅ Pairs well with {matching_items_count} item(s) in your wardrobe"
        )
    elif compatibility_score < 0.3:
        reasons.append(
            f"⚠️ Limited pairing options - only {matching_items_count} match(es)"
        )
    ...
    if is_duplicate:
        ...
        and body_shape_score >= 0.7
        and matching_items_count > 0
    ):
        recommendation = "buy"
        reasons.append("🎉 This is a great addition to your wardrobe!")
    elif (
        compatibility_score < 0.3 or body_shape_score < 0.4 or matching_items_count == 0
    ):
        recommendation = "skip"
        ...
        reasons.append(
            "🤔 Decent option, but not perfect - consider your budget and needs"
        )
...
def analyze_shopping_item(
    ...
    _validate_wardrobe_items(wardrobe_items)
    user_body_shape = (
        user_body_shape.strip() if isinstance(user_body_shape, str) else user_body_shape
    )
    user_undertone = (
        user_undertone.strip() if isinstance(user_undertone, str) else user_undertone
    )

    # Step 1: Classify the new item
    classification = classify_clothing(image)

    new_item_color = _require_non_empty_string(
        classification.get("color_primary"), "classification.color_primary"
    )
    ...
    compatibility = calculate_wardrobe_compatibility_score(
        matching_items=matching_items, total_relevant_items=total_relevant
    )

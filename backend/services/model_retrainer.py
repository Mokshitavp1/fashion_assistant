...
def retrain_color_harmony_rules(
    db: Session, feedback_data: Dict[str, Any], min_feedback: int = 100
    ...
    if len(ratings) < min_feedback:
        logger.info(
            f"Insufficient outfit ratings ({len(ratings)}) for color retraining"
        )
        return None
    ...
    try:
        ...
        high_rated_patterns = {
            ...
            "avg_rating": (
                sum(r.rating for r in high_rated) / len(high_rated) if high_rated else 0
            ),
        ...
        low_rated_patterns = {
            "count": len(low_rated),
            "avg_rating": (
                sum(r.rating for r in low_rated) / len(low_rated) if low_rated else 0
            ),
        ...
        logger.info(
            f"Color harmony retraining complete: {len(high_rated)} high-rated, {len(low_rated)} low-rated patterns"
        )
...
def retrain_clothing_classifier(
    db: Session, feedback_data: Dict[str, Any], min_feedback: int = 50
) -> Optional[Dict[str, Any]]:
    """
    ...
    if len(usage) < min_feedback:
        logger.info(
            f"Insufficient item usage data ({len(usage)}) for classifier retraining"
        )
        return None
    ...
    try:
        ...
        logger.info(
            f"Clothing classifier retraining complete: {kept_count} kept, {discarded_count} discarded, {worn_count} worn"
        )
...
def retrain_body_shape_detection(
    db: Session, feedback_data: Dict[str, Any], min_feedback: int = 30
) -> Optional[Dict[str, Any]]:
    """
    ...
    if len(rec_feedback) < min_feedback:
        logger.info(
            f"Insufficient body shape feedback ({len(rec_feedback)}) for retraining"
        )
        return None

    logger.info(
        f"Retraining body shape detection with {len(rec_feedback)} feedback points..."
    )
    ...
    try:
        ...
        logger.info(
            f"Body shape detection retraining complete: {helpful_rate:.1%} helpful rate"
        )
        return new_config
...
def evaluate_and_improve(
    ...
    try:
        ...
        old_accuracy = sum(m.value for m in old_accuracy_metrics) / len(
            old_accuracy_metrics
        )
        ...
        if old_model == "outfit_scoring":
            ...
            high_rated = new_model_checkpoint.get("high_rated_patterns", {}).get(
                "samples", 0
            )
        ...
        candidate_version = str(
            new_model_checkpoint.get("version", utcnow().strftime("%Y%m%d_%H%M%S"))
        )
        record_experiment(
        ...
        if should_deploy:
            logger.info(
                f"✓ New {old_model} model shows {improvement_estimated:.1%} improvement (threshold: {accuracy_threshold:.1%}), approving deployment"
            )
        else:
            logger.info(
                f"✗ New {old_model} model shows {improvement_estimated:.1%} improvement (threshold: {accuracy_threshold:.1%}), keeping old model"
            )
...
def deploy_model_if_improved(
    db: Session, model_name: str, new_checkpoint: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def retrain_all_models(
    db: Session, lookback_days: int = 30, min_feedback_threshold: int = 100
) -> Dict[str, Any]:
    """
    ...
    if total_feedback < min_feedback_threshold:
        logger.info(
            f"Insufficient feedback ({total_feedback} < {min_feedback_threshold}) to retrain"
        )
        return {
            "status": "skipped",
    ...
    logger.info(
        f"Retraining with {total_feedback} feedback points from last {lookback_days} days"
    )
    ...
    if new_classifier:
        if evaluate_and_improve(db, "clothing_classifier", new_classifier):
            deployment = deploy_model_if_improved(
                db, "clothing_classifier", new_classifier
            )
            results["registry_updates"].append(deployment)
